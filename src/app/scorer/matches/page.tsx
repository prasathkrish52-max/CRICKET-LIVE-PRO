...
export default function ScorerMatchListPage() {
  ...
  const fetchMatches = useCallback(async () => {
    try {
      ...
      console.error("Error fetching matches for scoring:", err);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {loading ? (
          <div className="grid gap-6">
            {[1, 2, 3].map(i => (
              <div key={i} className="skeleton h-32 w-full rounded-2xl bg-white/5 animate-pulse" />
            ))}
          </div>
        ) : error ? (
          <Card className="py-20 text-center border-red-500/20 bg-red-500/5">
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex-grow">
                      ...
                      <div className="flex items-center gap-4">
                        <span className="text-lg font-black text-white uppercase truncate">{match.team_a?.name || 'TBD'}</span>
                        <span className="text-stadium-gold font-black italic text-xs">VS</span>
                        <span className="text-lg font-black text-white uppercase truncate">{match.team_b?.name || 'TBD'}</span>
                      </div>
